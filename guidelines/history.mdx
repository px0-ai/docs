...
Every `##` heading in a guideline is a claim with its own id and version chain, addressed as `<path>#<heading-slug>` - so a claim that changed reads back on its own rather than as a diff of the whole file.
