...
Files px0 did not write carry no kind, so `--kind` never matches them - in a vault that is most of your brain. When a `--kind` search finds nothing, the output says so explicitly rather than looking like an empty brain.
...
`ask` is deliberately narrow. It touches no tools and no guidelines - it is retrieval plus generation over your brain and nothing else. If you want a question answered *and* posted somewhere, that is a workflow.
