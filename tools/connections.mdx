...
A workflow reaches outside px0 through a *tool*, and every tool routes through [Composio](https://composio.dev). There is no per-app connect command in px0, and that is a deliberate choice: you set one API key, and after that **apps authorize themselves**. The first time a workflow needs Gmail, px0 prepares Gmail's authorization and hands you the URL to approve.
...
Once stored, that bundle is used for **every** outbound request px0 makes, not only Composio ones. A `px0 knowledge add` against an internal wiki behind the same proxy works without further configuration, and if you ever need to set it by hand:
...
| :--- | :--- | :--- |
| `ready` | Authorized and usable | Nothing |
| `not authorized` | Never approved | Nothing — the next run that needs it offers a link |
...
| `authorization failed` | Composio rejected it | Re-run the workflow for a fresh link |
| `authorization gone` | px0 holds an id Composio no longer knows about | Re-run the workflow for a fresh link |
