...
You never have to know which apps exist, and you never connect an app you turn out not to need.
...
If an authorization later breaks - revoked in the app, deleted in Composio - the next run that needs it offers a fresh link. There is nothing to remove or reset by hand.
...
`--status` costs one API call per app, which is why it is opt-in rather than the default. See [Tools](/tools/catalogue) for the rest of what that listing shows.
