...
Dumb is the goal here. There is no queue to inspect, no job state to reconcile, and nothing to recover from a corrupt scheduler state - the workflow files *are* the schedule.
...
<Note>
  A scheduled workflow must write to a file (`output: {target: file, path: ...}`) or to your [inbox](/runs/inbox) (`output: {target: inbox}`). Nothing is watching stdout when the run fires, so px0 treats stdout-plus-schedule as a validation error rather than losing the output.
</Note>
...
```bash
px0 config set schedule.timezone Asia/Kolkata     # store-wide default
```
...
The first poll only records a baseline, so pointing a watch at a busy source does not immediately fire on everything already there. Composio's own event triggers need a public endpoint to deliver to, which a laptop does not have - this is what a local-first tool does instead.

What was new is piped to the run on stdin, so the workflow acts on the items that triggered it rather than going back to look and getting a different answer. `min_items` waits until that many have turned up; anything held back still counts on the next poll.
...
See [Workflow files](/workflows/anatomy#watching-instead-of-scheduling).
...
An unattended workflow that fails the same way `runs.disable_after_failures` times in a row (5 by default) is parked, and you are told through the same channel failures use.
...
```
px0: friday-pr-digest has been disabled
parked automatically after 5 consecutive failures: not authenticated
It will not fire again until you re-enable it: `px0 workflows enable friday-pr-digest`
```
...
Set `px0 config set runs.disable_after_failures 0` to let a workflow keep trying forever.
