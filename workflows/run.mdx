...
| :--- | :--- |
| `0` | Success |
| `1` | User error — bad arguments, no such workflow, no store, or a failed run |
...
A run that fails for any reason exits `1`, with the reason on stderr and the full detail in the run record. Codes `2` and `3` come from connector and harness failures raised outside a run — during a build, or while setting up a connection — so a script can tell "your Slack consent lapsed" apart from "this workflow is wrong".
...
px0 runs output <run-id>     # what it produced
