...
| `output` | map | `target: stdout`, `target: file` with a `path`, or `target: inbox`. `inbox: true` also delivers a file output to your inbox. |
...
| `pipeline` | list | Workflow ids to run in sequence instead of a prompt, or mappings with a `when:`. See below. |
| `confirm` | bool or list | `true` holds every write this workflow makes for approval; a list names only some. Overrides `tools.confirm_writes` in both directions. |
| `capture` | bool | Keep what this run's inputs resolved to, so a revision can be replayed against the same data. Overrides `runs.capture_inputs`. |
...
Three constraints govern `path`, and the reason for all of them is that the path can come from a model-written plan rather than from you:
...
```yaml
output: {target: inbox}
output: {target: file, path: "digests/pr-{date}.md", inbox: true}
```

<Note>
  A workflow with `trigger.schedule` or `trigger.watch` must use `target: file` or `target: inbox`. Nobody is watching stdout when a scheduled run fires, so px0 rejects that combination at validation time instead of dropping the output.
</Note>

`target: inbox` delivers the output to [your inbox](/runs/inbox) rather than to a file. `inbox: true` alongside a file target does both - the file is written, and the inbox tells you it exists. Scheduled and watched runs deliver automatically unless `inbox: false` says otherwise.
...
The run still happens and still produces its output; only the call that would leave a mark waits, in [`px0 approvals`](/approvals/overview), with its arguments shown in full.

Naming a tool the workflow cannot call fails validation rather than being ignored. The failure is silent in the dangerous direction - a misspelling would leave the tool you meant to hold back firing without asking.
...
A stage that fails aborts the pipeline, and the run is recorded as failed. Pipelines cannot nest - a stage that is itself a pipeline is a validation error, which keeps the run graph one level deep and legible.

### Skipping a stage

A stage can say when it should run:
...
`always` (the default), `has_output`, and `no_output` - three conditions, all facts about the previous stage's output that px0 can check itself. Anything richer would be a small language living in frontmatter, and the place for judgement about what to do next is a workflow body, which is written in English and read by a model.

A stage whose condition is not met is **skipped, not failed**, and the text it would have received passes through - so "post it only if there is something to post" does not break every stage after it. A condition on the first stage fails validation: there is no previous output to test, so it can only be a mistake about which stage it belongs to.
...
| A scheduled workflow writes to a file or the inbox | Otherwise its output goes nowhere |
| `output.target` is `stdout`, `file`, or `inbox`, and `file` has a `path` | No ambiguous destination |
| Pipeline stages exist, are not themselves pipelines, and name a known `when:` | Keeps the graph one level deep and its conditions checkable |
| Every tool in `confirm:` is one the workflow has | A misspelling would leave that tool firing without asking |
| `trigger.timezone` names a zone this machine knows | A silent fallback to machine time fires at the wrong hour |
