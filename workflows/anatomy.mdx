...
| `trigger` | map | `manual: true` and/or `schedule: "<cron>"`. |
...
| `timeout` | duration | Wall-clock cap on the run. Defaults to `120s`; accepts `ms`, `s`, `m`, `h`. |
...
<AccordionGroup>
  ...
  <Accordion title="retrieve — query your knowledge library">
    ```yaml
    inputs:
      - id: prior_art
    ...
    Runs a retrieval query over `knowledge/` and interpolates the matching passages, each prefixed with its `path#anchor` so the model can cite them. `k` defaults to `retrieval.k_default` (5).

    Anything under `knowledge/work/` is excluded from this, as it is from every other retrieval px0 performs.
  </Accordion>
...
px0 versions revert workflows/friday-pr-digest.md --to 2
```
