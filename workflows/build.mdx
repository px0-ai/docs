...
Behind that one command, px0 runs four model passes, each with a job small enough to do well, and stops for your confirmation at the two points where a wrong answer would actually cost you something:

1. **Clarify** — what is ambiguous about the request?
...
`px0 workflows new` on its own opens an interview instead, one question at a time, until px0 has what a workflow file must pin down:
...
The request is shown before the build spends a planning call on it: `edit` rewrites it, `n` cancels, anything else builds. Because the interview already settled these questions, the clarifying round below is skipped — you are not asked twice.

Not available with `--yes`, or when stdin is not a terminal: there is nobody to interview, so px0 says so and exits rather than building from nothing.

A description worth writing carefully is a paragraph, and a paragraph does not survive shell quoting well — `--from-file` reads it from a file instead:

```bash
px0 workflows new "" --from-file ./friday-digest.txt
```

## 1. It asks what is ambiguous
...
| Flag | Effect |
| :--- | :--- |
| `--yes` | Skip every prompt: interview, clarifying questions, tool confirmation, authorization, guideline authoring |
| `--id <id>` | Name the workflow directly instead of being asked |
| `--no-clarify` | Build from the description as written, without asking questions |
| `--no-discover` | Use only px0's curated tools; skip the catalogue search |
| `--from-file <path>` | Read the description from a file instead of the command line |

`--yes`, `--no-clarify`, and `--no-discover` all work on `px0 workflows edit` too, except `--id`, since an edit keeps the id it already has.
