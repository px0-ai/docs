...
px0 shapes what you wrote into `##` sections, shows you the draft, and takes `again` if you want another pass. What gets saved is an ordinary guideline file: it has version history from v1, so `px0 guidelines log` and `px0 guidelines revert` work on it, and it is added to this workflow's `guidelines:` list — which means its text is inlined into every run from now on.
...
| Flag | Effect |
| :--- | :--- |
| `--yes` | Skip every prompt: clarifying questions, tool confirmation, authorization, guideline authoring |
