...
- **Node.js 20 or newer**, only if you want [agent skills](/skills/agent-skills) or the [qmd retrieval backend](/knowledge/retrieval).
...
This scaffolds `~/.px0` (or `$PX0_HOME` if set): `workflows/`, `guidelines/`, `knowledge/`, `output/`, plus `config.toml` and a set of starter guideline files you can read, edit, or delete like any other file. Nothing here is hidden or binary.
...
That removes the binary and then tells you how to delete your store. It never deletes your data for you:
