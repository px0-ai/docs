...
This removes the binary, tears down the scheduler, and then *tells you* how to delete your store:

```
Removed the launchd scheduler unit.
To remove all local configurations and history, run:
  rm -rf ~/.px0
```

The scheduler unit goes before the binary does, because a launchd job with `KeepAlive` — or a systemd service — would otherwise keep trying to run a px0 that is no longer installed. launchd plists and systemd user units are removed for you; a cron entry cannot be edited safely on your behalf, so px0 tells you it is still there:

```
Note: px0 cron entries remain in your crontab; remove them with `crontab -e`.
```

By default, `px0 uninstall` never deletes the store itself. `~/.px0` is a plain directory of Markdown and TOML — your guidelines, your library, your run history — so removing it is your call, and it is yours to back up or move somewhere else first:

```bash
px0 store export ~/px0-backup
```

To remove the store, configurations, and history at the same time:
