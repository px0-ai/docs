...
`--quick` skips the three slower checks (daemon, harness, index) that need a live subprocess or a filesystem walk. `--json` gives you the whole report as data.
...
| `index` | The retrieval index is not empty while knowledge exists | Yes |

Two of those never fail on purpose. A stopped daemon is a choice, not a fault, and being one release behind is not a broken store — reporting them as failures would train you to ignore `doctor` output.
...
<AccordionGroup>
  ...
  <Accordion title="index — knowledge exists but nothing is indexed">
    ```
    ✗ index  12 knowledge files, 0 indexed passages
    ```

    Until the index is built, `px0 knowledge ask` and `px0 knowledge search` have nothing to retrieve from:
  ...
  <Accordion title="unreferenced_guidelines — informational, not a fault">
    ...
    Every freshly initialized store reports this, because `px0 init` scaffolds guidelines but no workflows. It is a nudge to prune, not a problem:
...
```bash
px0 runs why <run-id>
px0 versions diff guidelines/summarization.md 4 5
```
